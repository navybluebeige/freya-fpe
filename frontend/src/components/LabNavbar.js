    ...
    <>
      <nav className="sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center gap-3">
          <Link to="/labo" className="text-2xl font-extrabold text-primary-600 tracking-tight shrink-0 no-underline">Freya</Link>
          <span className="hidden sm:block text-xs font-semibold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md shrink-0">Laboratoire</span>
            ...
            <div className="hidden lg:block relative" ref={userRef}>
              ...
              {userOpen && (
                <div className="absolute top-12 right-0 w-56 bg-white rounded-2xl border border-slate-200 shadow-dropdown z-50 overflow-hidden" onClick={() => setUserOpen(false)}>
                  ...
                  <div className="p-1.5">
                    <Link to="/labo/profile" className="flex items-center px-3 py-2 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 no-underline">Mon profil</Link>
          ...
          <div className="absolute top-16 left-0 right-0 bg-white border-b border-slate-200 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3 bg-slate-50">
              ...
              <div>
                ...
                <div className="text-xs text-slate-400 flex items-center gap-1">{user?.email}</div>
              </div>
              <span className="ml-auto text-xs font-semibold text-green-600 bg-green-50 border border-green-200 px-2 py-0.5 rounded-full">Labo</span>
            </div>
            <div className="py-2">
              {NAV_LINKS.map(link => (
