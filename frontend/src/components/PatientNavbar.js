        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 ml-auto shrink-0">
            ...
            <button onClick={() => navigate('/doctors')}
              className="hidden md:block bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors border-0 cursor-pointer font-sans">
              + Prendre RDV
            </button>
            ...
            <div className="hidden lg:block relative" ref={userRef}>
              ...
              {userOpen && (
                <div className="absolute top-12 right-0 w-56 bg-white rounded-2xl border border-slate-200 shadow-dropdown z-50 overflow-hidden">
                  ...
                  <div className="p-1.5">
                    <Link to="/patient/profile" onClick={() => setUserOpen(false)}
                      className="w-full flex items-center px-3 py-2 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors no-underline">
                      Mon profil
                    </Link>
        ...
        <div className="lg:hidden fixed inset-0 z-40 bg-slate-900/50 backdrop-blur-sm" onClick={() => setMenuOpen(false)}>
          <div className="absolute top-16 left-0 right-0 bg-white border-b border-slate-200 shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 py-3 border-t border-slate-100 flex gap-2">
              <button onClick={() => { navigate('/doctors'); setMenuOpen(false); }}
                className="flex-1 bg-primary-600 text-white border-0 rounded-xl py-2.5 text-sm font-bold cursor-pointer hover:bg-primary-700 font-sans transition-colors">
                + Prendre RDV
              </button>
              <button onClick={handleLogout}
                className="flex items-center gap-1.5 text-red-500 border border-red-200 bg-red-50 rounded-xl px-4 py-2.5 text-sm font-semibold cursor-pointer font-sans hover:bg-red-100 transition-colors">
                <LogoutIcon /> Déco
              </button>
            </div>
          </div>
